...
class Sim8051:
    def __init__(self, firmware: bytes, bank: int = 0, bridge=None, bridge_log=False):
        ...
        self._timer_rr = 0              # round-robin cursor for same-level pending timers
    ...
    def _xdata_set(self, addr, val):
        ...
        if addr >= _XFR_BASE:
            ...
            if _DDC1_BASE <= addr < _DDC1_BASE + _DDC1_SIZE:
    ...
    # 8051 interrupt table: (IE bit, enable check, flag setter, vector)
    # Priority order matches hardware (low index = high priority).
    _IRQ_TABLE = [
        # (ie_bit, flag_check,              flag_set,                          vector)
        (0, lambda s: s.xfr_get(0xFFB8)&0x08, lambda s: None, 0x0003),  # EX0: scaler int (ISR W1Cs FFB8 bit 3)
        (1, lambda s: s._timer_tick(0, 0x88, 0x10, 0x20), lambda s: s._sfr_set(0x88, s._sfr_get(0x88)|0x20), 0x000B),  # ET0
        (2, lambda s: s.xfr_get(0xFFB9)&0x08, lambda s: None, 0x0013),  # EX1: ext int 1 (ISR W1Cs FFB9 bit 3)
        (3, lambda s: s._timer_tick(1, 0x88, 0x40, 0x80), lambda s: s._sfr_set(0x88, s._sfr_get(0x88)|0x80), 0x001B),  # ET1
        (4, lambda s: s._sfr_get(0x98)&0x03,lambda s: None, 0x0023),  # ES: serial (SCON.RI|SCON.TI)
        (5, lambda s: s._timer_tick(2, 0xC8, 0x04, 0x80), lambda s: s._sfr_set(0xC8, s._sfr_get(0xC8)|0x80), 0x002B),  # ET2
        # WDT interrupt: enable in FFEB bit 2, fires when counter >= threshold (FFAE + FFB0[7:4])
        (-1, lambda s: s.xfr_get(0xFFEB)&0x04 and s._wdt_counter_reached(), lambda s: s._xfr_or(0xFFEB, 0x80), 0x0043),
    ]
    ...
    def _sync_hw_interrupts(self):
        ...
        if not self.bridge:
            return
        # Stub op 0x42 burst-reads FFB8/FFB9/FF00 in one I2C round-trip.
        self.bridge.i2c.write(self.bridge.slave, 0x42, [0x00])
        b8, b9, status = self.bridge.i2c.read(self.bridge.slave, 0x08, 3)
        if b8 & 0x08:
            self.xfr_set(0xFFB8, self.xfr_get(0xFFB8) | 0x08)
        if b9 & 0x08:
            self.xfr_set(0xFFB9, self.xfr_get(0xFFB9) | 0x08)
        self.xfr_set(0xFF00, status)
    ...
    def _check_interrupts(self):
        ...
        if self._timer_rr >= len(self._IRQ_TABLE):
            self._timer_rr = 0
        for idx, (ie_bit, check, set_flag, vector) in enumerate(self._IRQ_TABLE):
            if (ie_bit >= 0 and not (ie & (1 << ie_bit))):
                continue
            if idx < self._timer_rr:
                continue
            self._timer_rr = self._timer_rr + 1
    ...
    def run(self, max_steps=0, stop_on_ljmp=False, stop_addr=None, stop_bank=0):
        ...
        CLOCK_SAMPLE_INTERVAL = 500        # sample wall clock every N steps
        hb_next = start + HEARTBEAT_INTERVAL
        now = time.monotonic()
        irq_period = 0.001                 # 1 ms — interrupt poll
        irq_next = now + irq_period
